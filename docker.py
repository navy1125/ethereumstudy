...
def docker_run(t,name,image):
    ...
    if state == "Up":
	    cmd = ("docker exec -ti %s_%s /bin/bash")%(t,name)
    elif state == "Exited" or state == "Created":
	    cmd = ("docker start %s_%s")%(t,name)
	    docmd(cmd)
	    cmd = ("docker exec -ti  %s_%s /bin/bash")%(t,name)
    elif platform.system().lower() == "darwin":
	    cmd = ("docker run --cap-add=sys_ptrace -it --hostname=docker-`hostname`-`whoami` -h %s_%s -v %s:%s --name %s_%s %s_%s:latest /bin/bash")%(t,name,os.getenv("HOME"),os.getenv("HOME"),t,name,t,image)
    else:
	    cmd = ("docker run --cap-add=sys_ptrace -it --hostname=docker-`hostname`-`whoami` -h %s_%s -v /etc/localtime:/etc/localtime:ro -v %s:%s --name %s_%s %s_%s:latest /bin/bash")%(t,name,os.getenv("HOME"),os.getenv("HOME"),t,name,t,image)
    docmd(cmd)
